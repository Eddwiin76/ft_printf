...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/12/13 19:08:07 by tpierre           #+#    #+#             */
/*   Updated: 2020/03/13 03:55:28 by tpierre          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "printf.h"

int		ft_uint_0(t_printf **list)
{
...
void	ft_uminus_0(int n, t_printf **list)
{
	int len;
...
void	ft_flag_uint_more(t_printf **list, int n,
		unsigned int nbr, unsigned int toprint)
{
	...
	ft_putunbr_fd(toprint, 1);
	(*list)->nprinted += (*list)->len;
	if ((*list)->minus == 1 && (*list)->width > (*list)->len)
		(*list)->nprinted +=
				ft_putchar_space_fd((*list)->width - (*list)->len, 1);
}
...
void	ft_flag_uint(t_printf **list)
{
	...
	if (n == -1)
		(*list)->len++;
	ft_flag_uint_more(list, n, nbr, toprint);
}
